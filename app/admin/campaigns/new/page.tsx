    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {CAMPAIGN_TEMPLATES.map((t) => (
          ...
          >
            <p className="text-xs uppercase tracking-[0.16em] text-[#8DEBFF]">{t.campaign_type.replace('_', ' ')}</p>
            <p className="font-serif text-xl text-[#F6FAFC] mt-1">{t.title}</p>
            <p className="text-sm text-[#A9B8C6] mt-2">{t.tagline}</p>
            <p className="text-sm text-[#53D6FF] mt-3">Goal {formatCents(t.goal_cents)}</p>
          </button>
        ))}
      </div>
    </div>
