...
import { cuesToSrt, cuesToVtt, episodeCues, transcriptPlainText } from '@/lib/studio/transcript'
...
/** Transcript in the requested format. Timed cues come from a pasted VTT/SRT or from the
 *  browser-Whisper word timings (transcript_words). Plain prose alone only serves as
 *  text/plain — we never invent cue timings, which would give listeners wrong captions. */
export async function transcriptFile(request: Request, context: Params, format: 'vtt' | 'srt' | 'txt') {
  ...
  const cues = episodeCues(episode)
