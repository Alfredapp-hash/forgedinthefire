...
function emptyDesk(tablesMissing: boolean, reason?: string) {
  ...
  return {
    ...
    fundraisingCampaigns: [] as { id: string; title: string; slug: string; utm_campaign: string | null; status: string }[],
...
async function loadFitf() {
  ...
  let fundraisingCampaigns: { id: string; title: string; slug: string; utm_campaign: string | null; status: string }[] = []
  ...
  if (!tablesMissing) {
    try {
      const { data } = await supabase
        ...
        .select('id, title, slug, utm_campaign, status')
        .in('status', ['live', 'draft', 'scheduled', 'paused'])
        .order('updated_at', { ascending: false })
        .limit(50)
      fundraisingCampaigns = data ?? []
    } catch {
