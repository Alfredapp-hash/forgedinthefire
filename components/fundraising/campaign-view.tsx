...
import { ContentAdvisory } from '@/components/fundraising/content-advisory'
...
import { daysLeft, formatCents } from '@/lib/fundraising/progress'
...
export function CampaignPublicView({
  ...
  const matchingLabel = campaign.matching_enabled
    ? `${campaign.matching_sponsor || 'A sponsor'} matches ${campaign.matching_ratio}:1${campaign.matching_cap_cents ? ` up to ${formatCents(campaign.matching_cap_cents)}` : ''}`
    : null
