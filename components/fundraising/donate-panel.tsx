...
import { DEFAULT_AMOUNTS, zeffyCheckoutUrl } from '@/lib/fundraising/templates'
...
export function CampaignDonatePanel({
  ...
  const amounts = ((campaign.suggested_amounts || []).length ? campaign.suggested_amounts : DEFAULT_AMOUNTS) as SuggestedAmount[]
  ...
  function checkout() {
    ...
    }, campaign.zeffy_url || ZEFFY_DONATE_URL)
  ...
  return (
    <div className="rounded-2xl border border-[#27313B] bg-[#151B22] p-5 space-y-4">
      ...
      {!showReport && (
        <button type="button" onClick={() => setShowReport(true)} className="block w-full text-center text-xs text-[#8DEBFF]">
          I already gave — add me to the thermometer
        </button>
      )}
