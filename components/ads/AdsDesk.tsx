...
export function AdsDesk() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 print:max-w-none">
      ...
      {tab === 'ours' && (
        <Ours data={data} busy={busy} setBusy={setBusy} onSaved={() => { void load().catch(() => undefined) }} />
      )}
...
function Ours({
  ...
  const [form, setForm] = useState({
    ...
    utm_campaign: '',
    landing_url: '/donate',
    dignity_reviewed: true,
    graphic_detail_reviewed: true,
    identifying_info_reviewed: true,
  })
  ...
  async function createCampaign() {
    ...
    const json = await res.json()
    setBusy(false)
    if (!res.ok) return alert(json.error || 'Could not save')
    setOpen(false)
    onSaved()
  }
    ...
    <div className="space-y-4">
      ...
      {open && (
        <div className="bg-[#151B22] rounded-xl border border-[#27313B] p-5 grid md:grid-cols-2 gap-3">
          ...
          <Field label="Landing URL"><input className={input} value={form.landing_url} onChange={(e) => setForm({ ...form, landing_url: e.target.value })} /></Field>
          <div className="flex items-end">
            <button type="button" disabled={busy} onClick={() => void createCampaign()} className="px-4 py-2 rounded-xl bg-[#53D6FF] text-[#061016] text-sm font-semibold">Save campaign</button>
          </div>
          <p className="text-xs text-[#A9B8C6] md:col-span-2">Dignity gates default on. Do not run raid, shock, or identifying survivor creative.</p>
        </div>
      )}
      ...
      {data.campaigns.length === 0 ? (
        <div className="bg-[#151B22] rounded-xl border border-[#27313B] p-12 text-center">
          ...
          <p className="text-sm text-[#A9B8C6] mt-1">Add Meta, Google, or Grants campaigns, then log weekly spend and attributed gifts. Peer graphs are already on Overview.</p>
        </div>
      ) : (
        <div className="space-y-4">
  ...
  return (
    <div className="bg-[#151B22] rounded-xl border border-[#27313B] p-5 space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-3 print:hidden">
        <form className="grid grid-cols-2 gap-2" onSubmit={(e) => { e.preventDefault(); void post(`/api/admin/ads/campaigns/${c.id}/spend`, { spend_dollars: Number(spend.dollars), period_start: spend.start, period_end: spend.start, clicks: spend.clicks || null, impressions: spend.impressions || null }).catch((err) => alert(err.message)) }}>
        ...
        <form className="grid grid-cols-2 gap-2" onSubmit={(e) => { e.preventDefault(); void post(`/api/admin/ads/campaigns/${c.id}/returns`, { attributed_dollars: Number(ret.dollars), period_start: ret.start, period_end: ret.start, gift_count: Number(ret.gifts || 0), method: 'manual' }).catch((err) => alert(err.message)) }}>
